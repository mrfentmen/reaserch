...
def iter_pairs(N, shard_i=0, nshards=1):
    ...
    for k in range(s, e):
        yield unrank(k, N)
def exact_search(N, EXPS=EXPS, prefilter=True):
    """Counterexample search (exact equality). Uses C20+C21. Returns (detections, stats)."""
    det = []
    st = {"pairs": 0, "checks": 0, "c20": 0, "c21": 0}
    for A, B in iter_pairs(N):
        st["pairs"] += 1
        if prefilter and math.gcd(A, B) != 1: continue
        for xa in EXPS:
            for xb in EXPS:
                s = pow(A, xa) + pow(B, xb)
                for e in EXPS:
                    if math.gcd(math.gcd(xa, xb), e) > 2: st["c21"] += 1; continue
                    if s % MOD[e] not in RES[e]: st["c20"] += 1; continue
                    st["checks"] += 1
                    f = iroot(s, e)
                    if pow(f, e) == s:
                        k = cls_euc(A, B, int(f))
                        det.append((A, xa, B, xb, int(f), e, k))
    return det, st
