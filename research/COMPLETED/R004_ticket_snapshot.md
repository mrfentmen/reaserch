...
- DEPENDENCIES: none.
- STATUS: ACTIVE (first execution cycle — see `EXPERIMENTS/R004_001.md`).
- NEXT ACTION: Run classifier now.
- EVIDENCE: pending experiment record.
...
- HYPOTHESIS: `prime()` is correct for all n < 2³¹ (double-sqrt exactness argument) and an independent Miller–Rabin agrees on a dense sample to 478M.
...
- DEPENDENCIES: none.
- STATUS: QUEUED.
- NEXT ACTION: Write domain argument + differential harness.
- EVIDENCE: prior `/tmp` spot scripts are background, not the deliverable.
...
- DEPENDENCIES: none.
- STATUS: QUEUED.
- NEXT ACTION: `git log --all --oneline`, `git branch -a`, remote check, backup dir listing.
- EVIDENCE: none yet.
